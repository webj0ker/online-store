export const checkCreditCard = (cardnumber: string) => {
  //Error messages
  const ccErrors = []
  ...
  for (let i = 0; i < cards.length; i++) {
    ...
    if (prefixValid) {
      ...
      for (let j = 0; j < lengths.length; j++) {
        if (cardnumber.length === lengths[j].length) {
          lengthValid = true
        }
      }
    }
    ...
    if (lengthValid && prefixValid) {
      ...
      cardMaxLength = cards[i].length
      return response(true, null, cardCompany, cardMaxLength)
    }
  }
